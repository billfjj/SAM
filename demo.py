...
def parse_args():
    ...
    parser.add_argument('--image', default=r'D:\Pythonproject\data\DOTA\data\val\images\part1\images\P0003.png', type=str, help='path to image file')
    parser.add_argument('--det_config', default=r'configs/Detic_LI21k_CLIP_SwinB_896b32_4x_ft4x_max-size.py', type=str, help='path to det config file')
    parser.add_argument('--det_weight', default=r'../models/detic_centernet2_swin-b_fpn_4x_lvis-coco-in21k_20230120'
                                              r'-0d301978.pth', type=str, help='path to det weight file')
    ...
    # GroundingDINO param
    # parser.add_argument('--text-prompt', '-t', default='cat', type=str, help='text prompt')
    parser.add_argument('--text-prompt', '-t', default='Large-vehicle, Swimming pool, Helicopter, Bridge, Plane, Ship, '
                                                       'Soccer-ball field, Basketball court, Storage tank, Roundabout, '
                                                       'Tennis court, Baseball diamond, Harbor, Small-vehicle, '
                                                       'Ground track field', type=str, help='text prompt')
    parser.add_argument(
...
def draw_and_save(image,
    ...
    if with_mask:
        ...
        h1, w1 = masks.shape[-2:]
        masks_img = np.zeros(masks.shape[1:]).reshape(h1, w1, 1) * np.zeros((1, 1, 4))
        for mask in masks:
            if random_color:
            ...
            masks_img += mask_image
            plt.gca().imshow(mask_image)
            # plt.show()
        # cv2.imwrite('outputs/test_mask.jpg', mask_image)
        plt.gca().imshow(masks_img)
        plt.show()
    plt.axis('off')
...
def main():
    ...
    files, source_type = get_file_list(args.image)  # 输入的图像路径
    ...
    for image_path in files:
        ...
        # with open('D:/Pythonproject/data/DOTA/data/val/labelTxt/Val_Task2_gt/valset_reclabelTxt/'+image_name+'.txt') as f:
        is_file = os.path.splitext(args.image)[-1].lower() in IMG_EXTENSIONS
        if is_file:
            image_name = image_path.split('\\')[-1].split('.')[0]
            # 读取yolo格式的数据标签
            with open('D:/Pythonproject/data/DOTA/data/val/labelTxt/Val_Task2_gt/valset_reclabelTxt/'+image_name+'.txt') as f:
                ...
                for line in lines:
                    ...
                    tenosr = torch.tensor(line)
                    box = tenosr.index_select(0, torch.tensor([0, 1, 4, 5])).reshape(1, -1)
                    if len(boxs) == 0:
                        boxs.append(box)
                    else:
                        temp_tensor = boxs[0]
                        temp = torch.cat([temp_tensor, box], dim=0)
                        boxs = [temp]
            # 读取coco格式的数据标签
        pred_dict = dict(boxes=0, scores=None, labels=None, masks=None)
        pred_dict['boxes'] = boxs[0]
        ...
        image1 = cv2.imread('../images/test.jpg')
        ...
        draw_and_save(image, pred_dict, save_path, show_label=args.not_show_label)
        progress_bar.update()
